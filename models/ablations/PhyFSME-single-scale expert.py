...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # ===== Single-scale expert ablation: BEGIN =====
        # Parse configured FRFT analysis windows, then keep only the largest one.
        # This isolates the contribution of multi-scale experts without changing the
        # global spectral analysis path used by the full model.
        patch_sizes = getattr(configs, "patch_sizes", "")
        if isinstance(patch_sizes, str) and patch_sizes.strip():
            parsed_patch_sizes = [int(p.strip()) for p in patch_sizes.split(",") if p.strip()]
        else:
            parsed_patch_sizes = [self.seq_len, self.seq_len // 4, self.seq_len // 8]
        parsed_patch_sizes = [p for p in parsed_patch_sizes if p > 0 and self.seq_len % p == 0]
        ...
        if isinstance(initial_alphas, str) and initial_alphas.strip():
            parsed_initial_alphas = [float(a.strip()) for a in initial_alphas.split(",") if a.strip()]
        else:
            parsed_initial_alphas = [0.5] * len(parsed_patch_sizes)
        if len(parsed_initial_alphas) < len(parsed_patch_sizes):
            parsed_initial_alphas += [parsed_initial_alphas[-1]] * (len(parsed_patch_sizes) - len(parsed_initial_alphas))
        parsed_initial_alphas = parsed_initial_alphas[:len(parsed_patch_sizes)]

        keep_idx = max(range(len(parsed_patch_sizes)), key=lambda i: parsed_patch_sizes[i])
        self.patch_sizes = [parsed_patch_sizes[keep_idx]]
        self.initial_alphas = [parsed_initial_alphas[keep_idx]]
        # ===== Single-scale expert ablation: END =====
