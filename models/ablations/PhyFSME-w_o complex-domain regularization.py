...
class FRFTScaleBlock(nn.Module):
    ...
    def compute_physical_loss(self, x, curr_alpha):
        # ===== w/o complex-domain regularization ablation: BEGIN =====
        # Ablation: remove complex-domain physical consistency regularization.
        # Keep the complex FRFT representation and inverse transform unchanged,
        # but set the auxiliary regularization term to zero.
        loss = x.real.new_zeros(())
        # ===== w/o complex-domain regularization ablation: END =====
        return loss
