...
class ScaleMoE(nn.Module):
    def __init__(self, num_scales, seq_len):
        super().__init__()
        # ===== Average fusion instead of Scale-MoE ablation: BEGIN =====
        # Remove the content-aware router and use a parameter-free uniform mean.
        # Keep the class name so the rest of the model and scripts stay unchanged.
        self.num_scales = num_scales
        # ===== Average fusion instead of Scale-MoE ablation: END =====

    def forward(self, original_x, scale_outputs):
        # ===== Average fusion instead of Scale-MoE ablation: BEGIN =====
        stacked_outputs = torch.stack(scale_outputs, dim=-1)
        fused_output = stacked_outputs.mean(dim=-1)
        # ===== Average fusion instead of Scale-MoE ablation: END =====
        return fused_output
